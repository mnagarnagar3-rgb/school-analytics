...
# Using Gemini 2.5 Flash for high-speed, free analysis
GEMINI_API_KEY = "AIzaSyC7KcjkDlUPHlWdUpg3Ue3WDftSqiJgiLM" 
genai.configure(api_key=GEMINI_API_KEY)

def get_ai_feedback(data_summary, role="Student"):
    """Fetches stable AI feedback using the 2.5 Flash model."""
    try:
...
if df.empty:
    st.error("⚠️ 'Student_Marks.xlsx' not found. Please upload the data file.")
    st.stop()
...
if page == "🏠 Overview":
    ...
    st.markdown("---")
    st.subheader("🤖 Student AI Insights")
    if st.button("Generate Personalized Feedback"):
        with st.spinner("Analyzing student growth..."):
    ...
    if mode == "Multi-Student Comparison":
        ...
        selected = st.multiselect("Select Students:", sorted(base_df['Student_Name'].unique()), default=sorted(base_df['Student_Name'].unique())[:2])
    ...
    st.markdown("---")
    st.subheader("🧠 Teacher Performance Insights")
    if st.button("Generate Faculty Analysis"):
        with st.spinner("Analyzing class trends..."):
            teacher_data_str = t_pivot.to_string()
            feedback = get_ai_feedback(teacher_data_str, role="Teacher/Faculty")
            st.success(feedback)
